...
var zksyncEnvSchema = z.object({
  ZKSYNC_ADDRESS: z.string().min(1, "ZKsync Era address is required").refine((address) => isAddress(address, { strict: false }), {
    message: "ZKsync Era address must be a valid address"
  }),
  ZKSYNC_PRIVATE_KEY: z.string().min(1, "ZKsync Era private key is required").refine((key) => /^[a-fA-F0-9]{64}$/.test(key), {
    message: "ZKsync Era private key must be a 64-character hexadecimal string (32 bytes) without the '0x' prefix"
  })
});
...
import { zksync as zksync2, mainnet } from "viem/chains";
